...
namespace {
    ...
    // Count Palace objects in a region (any state, including incomplete).
    size_t count_palaces(ARegion *r) {
        size_t n = 0;
        for (const auto o : r->objects) if (o->type == O_PALACE) n++;
        return n;
    }
...
// Palace is the NewAge capital "throne" for the Trident victory mechanic. It is a
// SETTLEMENT_ONLY | ONE_PER_REGION building (cost 300 stone, BUILD 3), mirroring the
// Town Hall constraints. See docs/TRIDENT_VICTORY_MECHANIC_DESIGN.md.
ut::suite<"Palace Build"> palace_build_suite = [] {
    using namespace ut;

    "Palace builds successfully in a settlement"_test = [] {
        ...
        leader->items.SetNum(I_STONE, 300);
        ...
        expect(faction->errors.size() == 0_ul) << "no errors expected when building first Palace";
        expect(r->objects.size() == before + 1) << "a new building object must be created";
        expect(count_palaces(r) == 1_ul) << "exactly one Palace must exist";
    };

    "Cannot build a second Palace in the same region"_test = [] {
        UnitTestHelper helper;
        helper.initialize_game();
        helper.setup_turn();
        ...
        // Pre-existing completed Palace.
        helper.create_building(r, nullptr, O_PALACE);
        expect(count_palaces(r) == 1_ul);

        builder->items.SetNum(I_STONE, 300);
        ...
        expect(count_palaces(r) == 1_ul) << "the second Palace must not be created";
    ...
    "Palace cannot be built in a region without a settlement"_test = [] {
        UnitTestHelper helper;
        helper.initialize_game();
        helper.setup_turn();
        ...
        expect(count_palaces(wild) == 0_ul);
